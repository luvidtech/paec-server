...
export const createBaselineForm = asyncHandler(async (req, res) => {
  try {
    ...
    // Additional debugging for file processing
    if (req.files?.mriImages) {
      console.log("Processing", req.files.mriImages.length, "files");
      req.files.mriImages.forEach((file, index) => {
        ...
        for (const file of mriFiles) {
          try {
            ...
            if (!allowedTypes.includes(file.mimetype)) {
              return res.status(400).json({ 
                message: `Invalid file type: ${file.originalname}. Only JPEG, PNG, GIF, and WebP images are allowed.` 
              });
            }
            ...
            const newFileName = `${paecNo}_${uniqueId}${path.extname(
              originalName
            )}`;
    ...
    const newForm = new BaselineForm(finalFormData);
...
export const getBaselineForm = asyncHandler(async (req, res) => {
  try {
    const { page, limit, sex, minAge, uhid, name, visitDate } = req.query;
...
export const getBaselineFormById = asyncHandler(async (req, res) => {
  try {
    ...
    const isSameCenter =
      form.center?.toString() === req.user.center?.toString();
...
export const searchBaselineForm = asyncHandler(async (req, res) => {
  try {
    const { page = 1, limit = 10, paecNo, uhid, name } = req.query;
    ...
    if (searchConditions.length === 0) {
      return res.status(400).json({
        message:
          "At least one search parameter (paecNo, uhid, or name) is required",
      });
    }
    ...
    if (req.files && Object.keys(req.files).length > 0) {
      ...
      if (mriFiles && Array.isArray(mriFiles)) {
        for (const file of mriFiles) {
          ...
          const uniqueId = uuidv4();
          const paecNo =
            patientDetails?.paecNo || form.patientDetails?.paecNo || "unknown";
          const newFileName = `${paecNo}_${uniqueId}${path.extname(
            originalName
          )}`;
          const filePath = path.join(mriFolder, newFileName);
          ...
          if (fs.existsSync(file.path)) {
            ...
            return res
              .status(400)
              .json({ message: "File not found for upload" });
          }
        }
      }
    }
...
export const deleteBaselineForm = asyncHandler(async (req, res, next) => {
  const form = await BaselineForm.findById({
    _id: req.params.id,
    "isDeleted.status": false,
  });
  ...
  if (form) {
    ...
    res
      .status(200)
      .json({ message: "Baseline form and related follow-ups deleted" });
  } else {
